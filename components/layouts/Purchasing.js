...
const Purchasing = () => {
    ...
    return (
        <div className='py-[120px] max-w-[90%] lg:max-w-[1440px] lg:px-[120px] m-auto grid gap-[50px]'>
            <div className='grid gap-[20px] pb-[50px] border-b-[2px] border-black'>
                ...
                <p className="max-w-[580px] text-[36px] lg:text-[56px] text-[#1e1d1d] font-bold leading-[48px]">Order solar panels in three steps</p>
            </div>
            <div className='grid gap-[40px]'>
                {steps.map((step, index) => (
                    <div
                        key={index}
                        className='grid grid-flow-col justify-between items-center pb-[50px] border-b-[1px]'
                        onMouseEnter={() => setHoveredIndex(index)}
                        onMouseLeave={() => setHoveredIndex(null)}
                    >
                        <div className='grid grid-flow-col gap-[60px] items-start'>
                        ...
                        <div className='grid grid-flow-col gap-[60px] items-start'>
                            ...
                            <Link href='#' className={`p-5 bg-[#337ab740] rounded-full transform transition-transform duration-300 ${hoveredIndex === index ? '-rotate-45' : ''}`}>
